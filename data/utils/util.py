...
def get_pickle_data_args(processed_data_args):
    return json.load(open(processed_data_args, "r"))
    
def get_dataset(
    dataset: str, processed_data_dir, client_id: int, batch_size=32, valset_ratio=0.1, testset_ratio=0.1,
) -> Dict[str, Subset]:
    _ARGS_DICT = get_pickle_data_args(os.path.join(processed_data_dir, "args.json"))
    client_num_in_each_pickles = _ARGS_DICT["client_num_in_each_pickles"]
    pickles_dir = os.path.join(processed_data_dir, "pickles")
...
def get_client_id_indices(
    ...
    _ARGS_DICT = get_pickle_data_args(os.path.join(processed_data_dir, "args.json"))
    #pickles_dir = os.path.join(config["processed_data_dir"], "pickles")
    with open(os.path.join(processed_data_dir, "pickles", "seperation.pkl"), "rb") as f:
        seperation = pickle.load(f)
    if _ARGS_DICT["type"] == "user":
        # return seperation["train"], seperation["test"], seperation["total"]
        return seperation["train"], seperation["total"]
    else:  # NOTE: "sample"
